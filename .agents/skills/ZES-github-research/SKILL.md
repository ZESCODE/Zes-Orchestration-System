...
## 9. Browser Fallback (When API Rate Limited)

When `gh` or GitHub API returns rate-limit errors, use **browser-harness** to scrape GitHub directly.

### Quick Check
```bash
browser-harness <<'PY'
new_tab("https://github.com/owner/repo")
import time; time.sleep(3)
info = page_info()
print(f"Page: {info['title']}")
capture_screenshot("/tmp/github-repo.png")
PY
```
...
```bash
browser-harness <<'PY'
new_tab("https://github.com/search?q=llm+agent+memory&type=repositories&s=stars&o=desc")
import time; time.sleep(3)
print("Loaded:", page_info()["title"])
PY
```
...
|----------|-----------|-----|
| Rate limit available | `gh` CLI | Faster, structured JSON |
| Rate limited | browser-harness | Still works, slower |
| Private repo (no PAT) | browser-harness | Uses your browser session |
| Rendered README needed | browser-harness | Shows images, tables, formatting |

## Pair With

- **browser-harness** — Browser fallback when API is rate limited
- **cdp-audit** — Debug browser scraping issues
